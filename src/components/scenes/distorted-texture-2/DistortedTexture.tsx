...
const createAttributes = () => {
  ...
  const coordinates = new Float32Array(number * 3)
  const speeds = new Float32Array(number)
  const offsets = new Float32Array(number)
  const direction = new Float32Array(number)
  const press = new Float32Array(number)
  ...
  for (let i = 0; i < width; i++) {
    ...
    for (let j = 0; j < height; j++) {
      ...
      positions[index * 3 + 1] = (j - width / 2) * 2
